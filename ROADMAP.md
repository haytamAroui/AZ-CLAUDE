...
### Decision Logic
- If no plan exists → run /dream then /plan
...
- If all milestones done → run /review on full project
...
### Completion
When all milestones are done and /review passes:
...
- References /dream, /plan, /evolve, /review, /ship

---

### PHASE 2: THE PLAN SYSTEM (Day 1 — 3 hours)
**What:** Enhance /plan to produce machine-readable plan.md

Current /plan outputs text. Copilot needs structured milestones it can track.

File: Update `templates/commands/plan.md` and add `templates/capabilities/shared/plan-tracker.md`

**plan.md format (generated by /plan):**
...
**Tests to add:**
- /plan generates plan.md with milestone structure
...
while [ $SESSION_COUNT -lt $MAX_SESSIONS ]; do
  ...
  if [ -f .claude/plan.md ]; then
    ...
    PROMPT="$PROMPT\n\nNo plan yet. Start with /dream then /plan."
  fi
...
**4B: /plan integration** 
- After /dream, copilot runs /plan
- /plan reads CLAUDE.md + intent → generates plan.md with milestones
...
**4E: /review integration (after all milestones)**
- When plan.md shows all milestones done, copilot runs /review
- /review does spec-first audit: does the build match the intent?
...
| 5 | templates/commands/plan.md (update) | Update | 1 hour | 2 |
...
| .claude/plan.md | Copilot (every session) | /plan, copilot | Milestone tracker |
| .claude/memory/goals.md | UserPromptSubmit hook | PostToolUse hook | File edit breadcrumbs |
| .claude/memory/checkpoints/* | UserPromptSubmit hook | /checkpoint | Reasoning snapshots |
...
| /review fails 3 times | Exit 1 — fundamental spec mismatch |
...
Step 2: Update templates/commands/plan.md for structured output
        → test: /plan generates plan.md with milestone format
