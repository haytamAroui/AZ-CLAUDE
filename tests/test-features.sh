...
check "adaptCommandContent function"             "$CLI" "adaptCommandContent"
check "adaptCommand: strips Claude-specific fields" "$CLI" "allowed-tools\|disable-model-invocation\|argument-hint"
check "adaptCommand: keeps description"          "$CLI" "description"
check "installCommands passes CLI name"          "$CLI" "installCommands.*cliName\|cliName.*Claude Code"
check "cli: --cli flag override"                 "$CLI" "cliFlag\|--cli.*flag\|--cli.*override"
check "cli: matches by exe name"                 "$CLI" "c\.exe.*cliValue\|exe.*===.*cliValue"
