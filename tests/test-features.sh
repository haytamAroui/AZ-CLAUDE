...
check      "plugin: marketplace version matches 0.4"      ".claude-plugin/marketplace.json" "0\.4\."
...
check      "router: fires on every message"                "$UP" "AZCLAUDE Brain Router.*fires on EVERY message\|fires on EVERY"
check      "router: intent detection BUILD"                "$UP" "build.*add.*create.*implement\|intents.push.*BUILD"
check      "router: intent detection FIX"                  "$UP" "fix.*bug.*broken\|intents.push.*FIX"
check      "router: intent detection REVIEW"               "$UP" "review.*audit.*safe\|intents.push.*REVIEW"
check      "router: intent detection FRONTEND"             "$UP" "frontend.*ui.*css\|intents.push.*FRONTEND"
check      "router: intent detection PLAN"                 "$UP" "plan.*blueprint.*architect\|intents.push.*PLAN"
check      "router: intent detection DEVOPS"               "$UP" "deploy.*ci.*docker\|intents.push.*DEVOPS"
check      "router: intent detection EXTEND"               "$UP" "agent.*skill.*create\|intents.push.*EXTEND"
check      "router: spawns problem-architect"              "$UP" "problem-architect.*pre-flight\|subagent_type.*problem-architect"
check      "router: spawns code-reviewer"                  "$UP" "code-reviewer.*review\|subagent_type.*code-reviewer"
check      "router: spawns test-writer"                    "$UP" "test-writer.*generate\|subagent_type.*test-writer"
check      "router: spawns security-auditor"               "$UP" "security-auditor.*111\|subagent_type.*security-auditor"
check      "router: loads frontend-design skill"           "$UP" "frontend-design.*SKILL.md"
check      "router: loads test-first skill"                "$UP" "test-first.*SKILL.md"
check      "router: loads architecture-advisor skill"      "$UP" "architecture-advisor.*SKILL.md"
check      "router: checks agent exists before routing"    "$UP" "agentExists\|agent.*Exists"
check      "router: checks skill exists before routing"    "$UP" "skillExists\|skill.*Exists"
check      "router: skips slash commands"                  "$UP" "startsWith.*/"
check      "router: DISPATCH output format"                "$UP" "AZCLAUDE DISPATCH"
check      "router: persists prompt for routing"           "$UP" "azclaude-prompt.*writeFileSync\|writeFileSync.*azclaude-prompt"
check      "router: Do NOT skip instruction"               "$UP" "Do NOT skip"
