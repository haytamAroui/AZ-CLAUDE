...
check "adaptFrontmatter function"                "$CLI" "adaptFrontmatter"
check "adaptFrontmatter: strips Claude fields"   "$CLI" "universally supported\|Claude-specific"
check "adaptFrontmatter: keeps description"      "$CLI" "description"
check "installAgents passes CLI name"            "$CLI" "installAgents.*cliName\|cliName.*Claude Code"
check "installCommands passes CLI name"          "$CLI" "installCommands.*cliName\|cliName.*Claude Code"
check "cli: --cli flag override"                 "$CLI" "CLI_OVERRIDE\|--cli.*flag\|--cli.*override"
check "cli: matches by exe name"                 "$CLI" "c\.exe.*===.*v\b\|exe.*===.*v\b"
check "cli: shows valid CLIs on unknown"         "$CLI" "Unknown CLI\|Valid:"
check "cli: banner shows --cli option"           "$CLI" "npx azclaude-copilot --cli"
